  ...
  if (test.attemptStatus === 'SUBMITTED') {
    return (
      <div className="mx-auto max-w-xl text-center py-20">
        <p className="text-lg font-semibold text-slate-800 dark:text-slate-100">You have already submitted this test.</p>
        <button
          onClick={() => navigate(ROUTES.STUDENT_RESULT(test.attemptId!))}
          className="mt-4 rounded-xl bg-brand-700 px-6 py-2.5 text-sm font-semibold text-white hover:bg-brand-800 dark:bg-brand-600 dark:hover:bg-brand-700"
        >
          View Result
        </button>
      </div>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="rounded-2xl border border-slate-100 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-800/60">
        ...
        {/* Rules */}
        <div className="mb-8 rounded-xl bg-slate-50 p-5 dark:bg-slate-900/60">
          <h2 className="mb-3 text-sm font-semibold text-slate-700 dark:text-slate-200">Important Rules</h2>
          <ul className="space-y-2 text-sm text-slate-600 dark:text-slate-300">
            <li className="flex gap-2">
              <span className="text-slate-400 shrink-0 dark:text-slate-500">•</span>
        ...
        <button
          onClick={handleStart}
          disabled={starting}
          className="w-full rounded-xl bg-brand-700 py-3 text-sm font-semibold text-white hover:bg-brand-800 transition-colors disabled:opacity-60 dark:bg-brand-600 dark:hover:bg-brand-700"
        >
          {starting
            ? 'Starting...'
            : isResume
...
function InfoCard({ icon, label, value }: { icon: string; label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 text-center dark:border-slate-700 dark:bg-slate-900/60">
...
function MarkCard({
  ...
  const colorMap = {
    green: 'bg-green-50 text-green-700 border-green-100 dark:bg-green-950/30 dark:text-green-400 dark:border-green-900',
    red: 'bg-red-50 text-red-600 border-red-100 dark:bg-red-950/30 dark:text-red-400 dark:border-red-900',
    slate: 'bg-slate-50 text-slate-500 border-slate-100 dark:bg-slate-900/60 dark:text-slate-400 dark:border-slate-700',
  };
  return (
