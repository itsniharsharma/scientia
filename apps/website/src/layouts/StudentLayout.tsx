...
export function StudentLayout() {
  ...
  const fullName = user && 'fullName' in user ? user.fullName : '';
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
        ...
        <nav className="flex-1 space-y-0.5 px-3 py-4">
          {NAV_ITEMS.map(({ to, label, icon }) => (
            <NavLink
              key={to}
              to={to}
              end={to === ROUTES.STUDENT_DASHBOARD}
              className={({ isActive }) =>
                [
        ...
        <div className="border-t border-slate-100 px-4 py-4 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium text-slate-800 dark:text-slate-100">{fullName || username}</p>
      ...
      {mobileOpen && (
        <div
          className="fixed inset-0 z-20 bg-black/40 md:hidden dark:bg-black/60"
          ...
          >
            ...
            <div className="mt-4 border-t border-slate-100 px-4 pt-4 dark:border-slate-800">
              <div className="flex items-center gap-2 pb-3">
                ...
                <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{fullName || username}</span>
              </div>
              <button
                onClick={handleSignOut}
